...
class character_LIST{
	...
	public:
		...
		void display()			{ for(auto i: mp) {	cout << "Ki tu " << i.first << " XUAT HIEN " << i.second << endl;} }
		bool ceasar_Encode()	{
		...
		vector <pair<char, int>> map_2_vector()		{
			...
			// for (auto i : RES) {
			// 	cout << "KI TU " << i.first << " Xuat hien:" << i.second << endl;
			// }
			return RES;
		}
...
class HUFF_NODE		{
	public:
		...
		HUFF_NODE(const char &val, const int &wgt) : val(val), wgt(wgt), lc(nullptr), rc(nullptr) {};  // TAO NODE LA
...
class HUFFMAN_TREE	{
	public:
		...
		void print_Tree(HUFF_NODE* root, string s) {
        if (!root) {
            cout << s << "null" << endl;
            return;
        }
        if (root->lc == nullptr || root->rc == nullptr) {
            cout << s << root->val << "/" << root->wgt << endl;
        }
        else cout << s << root->wgt << endl;
        s += "  ";
        s.push_back(char(179));
        print_Tree(root->lc, s);
        print_Tree(root->rc, s);
    }
...
class __GOJO___RESTAURANT{
		...
		class BST_TREE{
			...
			public:
				...
				int C (const int &k, const int &n)		{
					...
					return (C(k, n - 1) % MAXSIZE + C (k - 1, n - 1) % MAXSIZE ) % MAXSIZE;
				}
				int COUNTING_KOKUSHEN	(Node *& root) 	{
					if(!root || root->isLeaf()) return 1;

					int k,n = DEM_SO_PHAN_TU(root);

					if(root->lc && root->rc) k = DEM_SO_PHAN_TU(root->lc);
					else return 1;

					return COUNTING_KOKUSHEN(root->lc) * COUNTING_KOKUSHEN (root->rc) * C(k, n);
				}
				...
				bool DEL(Node *&root,const int &val){
					if(!root) return false; // ko the xoa
	...
	public:
		...
		void KOKUSEN_G()	{
			for(int i = 0, n = HASHTABLE.size(); i < n; i++){
...
class Node{
	public:
		...
		void print_LIFO	()	{ for(int n = RES_VAL.size(), i = n - 1; i >= 0; i--)	{ cout << TAG << "-" << RES_VAL[i] << endl;}}
...
class Comp	{
	public:
		static bool prior (const Node &A, const Node &B)	{
			...
			return (A.ne != B.ne) ? false : (A.PROCESSING < B.PROCESSING); 
		}
};
class ___HEAP{
	public:
		void swap(Node *ARR, const int &first, const int &second)    {
	...
	public:
		___HEAP()	{	heap = new Node[MAXSIZE]; maxsize = MAXSIZE; ne = 0; PROCESS = 0;}							
		...
		void DUYET_PREORDER		(int R)		{
			if(R < 0 || R >= ne) return;
			heap[R].print_LIFO();
			DUYET_PREORDER	(this->leftchild(R));
			DUYET_PREORDER	(this->rightchild(R));
		}
		void removefirst		()				{
			swap(heap, 0, --ne);
		...
		void KEITEIKEN_SH	(const int &num){
			for(int i = 0 ; i < min(num, MAXSIZE); i++)	{
				heap[i].KEITEIKEN_H(num, ++PROCESS);
			}
			while	(ne && heap[0].ne == 0)	{ removefirst();}
		}
		int size 			()				{ return ne;}
		~___HEAP			()				{ delete [] heap;}
};
...
void print (priority_queue <HUFFMAN_TREE *, vector <HUFFMAN_TREE *>, Compare_HUFF> PQ)	{
	while(!PQ.empty()) {
		HUFFMAN_TREE *TEMP = PQ.top();
		PQ.pop();
		TEMP->print_Tree(TEMP->ROOT, "");
	}
}
...
class imp_res{
	...
	public:
		...
		void LAPSE(string name)						{ 
			...
			cout << A_TREE->DECODE_VAL << endl;
			CHOOSE_RESTAURANT(A_TREE->DECODE_VAL);
		}
		void KOKUSEN()			{ G->KOKUSEN_G();}
		void KEITEIKEN(int num)	{ S->KEITEIKEN_S(num);   /* xem lai ham nay*/}
		void HAND()				{ TREE->print(TREE->ROOT);  }
		void LIMITLESS(int num)	{ G->print(num);}
		void CLEAVE(int num)	{ S->print();}
		~imp_res()				{
			if(TREE) {
				TREE->delete_tree(TREE->ROOT);
				delete TREE;
